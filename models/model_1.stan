...
model {
  ...
  b_interaction ~ cauchy(0, 1);
  // model
  
  for (i in 1:n_train) {
    
    mu = economy_train[i]*b_economy + corruption_train[i]*b_corruption + economy_train[i]*corruption_train[i]*b_interaction + intercept;
    // model
...
generated quantities {
  ...
  for (i in 1:n_train) {
    ...
    real mu = economy_train[i]*b_economy + corruption_train[i]*b_corruption + economy_train[i]*corruption_train[i]*b_interaction + intercept;
    score_pred_train[i] = mu;
  ...
  for (i in 1:n_test) {
    ...
    // sum polynomial terms together
    score_pred_test[i] = economy_test[i]*b_economy + corruption_test[i]*b_corruption + economy_test[i]*corruption_test[i]*b_interaction + intercept;
